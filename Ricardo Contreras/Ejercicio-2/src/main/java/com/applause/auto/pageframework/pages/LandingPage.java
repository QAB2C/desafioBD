...
public abstract class LandingPage extends AbstractPage {
	 ...
	 * @param destinationName
	 */
	public void enterDestinationAndConfirmKaleido(String destinationName) {
		LOGGER.info(String.format("Entering destination %s into search field", destinationName));
		getOpenDestination().click();
		getDestinationInputText().click();
		getDestinationInputText().setText(destinationName);
		syncHelper.waitForElementToAppear(getDropdownAjax());
		// getDropdownAjax().click();
	}
	 ...
	 */
	public void enterGetDestinationList() {
		LOGGER.info(String.format("Getter and Print Destination List"));
		System.out.println(String.format(
				"\n ***************Lista de Destinos***************** \n %s \n ****************************************************",
				getDropdownAjax().getText()));
	}
	...
	protected EditField getOpenDestination() {
		return new EditField(this, getLocator(this, "getOpenDestination"));
	}

	@WebElementLocator(webDesktop = "#EtDestinyHtl")
	protected EditField getDestinationInputText() {
		return new EditField(this, getLocator(this, "getDestinationInputText"));
	}

	@WebElementLocator(webDesktop = "#ui-id-1")
	protected Link getDropdownAjax() {
		return new Link(this, getLocator(this, "getDropdownAjax"));
	}
