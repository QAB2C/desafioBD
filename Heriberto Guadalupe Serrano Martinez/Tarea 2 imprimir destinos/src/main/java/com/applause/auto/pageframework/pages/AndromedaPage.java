...
public abstract class AndromedaPage extends AbstractPage {
	...
	public void enterDestination(String destinationName) {
		...
		syncHelper.waitForElementToAppear(getDestinationListBox());
		System.out.print(getDestinationListBox().getWebElement().getText());
	...
	@WebElementLocator(webDesktop = ".form-control")
	...
	@WebElementLocator(webDesktop = ".container-fluid")
	protected Dropdown getDestinationListBox() {
		return new Dropdown(this, getLocator(this, "getDestinationListBox"));
	}
	 ...
	 */
	/*
	 * private void dismissPopup() { try { LOGGER.info("Attempting to dismiss popup");
	 * syncHelper.waitForElementToAppear(getPopupWelcomeText()); getDismissPopupButton().click(); } catch
	 * (NoSuchElementException e) { LOGGER.info("Popup not found, moving on"); }
	 * 
	 * }
	 */
}
